...
const DeveloperValues = ({ developerValues }: DeveloperValuesProps) => {
    return (
        <div className="w-full h-[320px] mt-8 rounded-lg bg-[#F7F7F7] border border-[#E2E2E2] bg-opacity-90">
            <div className="w-full p-4">
                {developerValues?.map((item, index) => (
                    <div key={index} className="p-2">
                        <p className="font-bold text-lg py-1">{item.title}</p>
                        <p className="text-sm font-normal whitespace-pre-line">{item.description}</p>
                    </div>
                ))}
            </div>
        </div>
